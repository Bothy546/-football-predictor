...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loading.setText("⚽ Loading 49 worldwide leagues...\n🤖 Checking past tickets...");
        ...
        executor.execute(() -> {
            try {
                ...
                if (!yesterday.isEmpty() && !yesterday.equals(today)) {
                    String[] types = {"safe", "medium", "risky", "smart", "value", "extreme1", "extreme2"};
    ...
    private void displayMatches(String jsonData) {
        try {
            ...
            if (matches.length() == 0) {
                ...
                noData.setText("⚠️ No matches available right now.\nTry again later or check your API key!");
            ...
            for (int i = 0; i < matches.length(); i++) {
                try {
                    ...
                    String homeTeam = match.optString("home_team", "");
                    String awayTeam = match.optString("away_team", "");
                    String dateTime = match.optString("commence_time", "");
                    
                    if (homeTeam.isEmpty() || awayTeam.isEmpty() || dateTime.isEmpty()) continue;
                    ...
                    JSONArray bookmakers = match.optJSONArray("bookmakers");
                    if (bookmakers == null || bookmakers.length() == 0) continue;
                    
                    JSONObject bookmaker = bookmakers.getJSONObject(0);
                    JSONArray markets = bookmaker.optJSONArray("markets");
                    ...
                    String time = dateTime.length() >= 16 ? dateTime.substring(11, 16) : "TBA";
            ...
            if (allMatches.isEmpty()) {
                ...
                noMatches.setText("⚠️ No valid matches found in next 3 days.\nCheck back later!");
                noMatches.setTextSize(16);
                noMatches.setPadding(20, 20, 20, 20);
            ...
            TextView error = new TextView(this);
            error.setText("❌ Error processing matches: " + e.getMessage());
            error.setTextColor(Color.RED);
    ...
    private void displayStats() {
        ...
        int valueWins = prefs.getInt("value_wins", 0);
        int valueLosses = prefs.getInt("value_losses", 0);
        int extreme1Wins = prefs.getInt("extreme1_wins", 0);
        int extreme1Losses = prefs.getInt("extreme1_losses", 0);
        int extreme2Wins = prefs.getInt("extreme2_wins", 0);
        int extreme2Losses = prefs.getInt("extreme2_losses", 0);
        ...
        statsCard.addView(createStatText("💰 Value Hunter: " + valueWins + "W / " + valueLosses + "L " + 
            (valueWins + valueLosses > 0 ? "(" + (valueWins * 100 / (valueWins + valueLosses)) + "%)" : "")));
        statsCard.addView(createStatText("🔥 Extreme Tier 1: " + extreme1Wins + "W / " + extreme1Losses + "L " + 
            (extreme1Wins + extreme1Losses > 0 ? "(" + (extreme1Wins * 100 / (extreme1Wins + extreme1Losses)) + "%)" : "")));
        statsCard.addView(createStatText("💥 Extreme Tier 2: " + extreme2Wins + "W / " + extreme2Losses + "L " + 
            (extreme2Wins + extreme2Losses > 0 ? "(" + (extreme2Wins * 100 / (extreme2Wins + extreme2Losses)) + "%)" : "")));
    ...
    private void displayDailyTickets() {
        ...
        ArrayList<SmartPick> valueMatches = new ArrayList<>();
        ArrayList<SmartPick> extreme1Matches = new ArrayList<>();
        ArrayList<SmartPick> extreme2Matches = new ArrayList<>();
        ...
        for (MatchPrediction m : allMatches) {
            if (m.dateLabel.equals("📅 TODAY")) {
                ...
                SmartPick value = getBestValuePick(m);
                if (value.odds >= 2.0 && value.odds <= 2.8 && value.prob >= 62 && valueMatches.size() < 4) {
                    valueMatches.add(value);
                }
                
                SmartPick extreme = getBestHighOddsPick(m);
                if (extreme.odds >= 2.0 && extreme.prob >= 60) {
                    if (extreme1Matches.size() < 5) extreme1Matches.add(extreme);
                    if (extreme2Matches.size() < 7) extreme2Matches.add(extreme);
                }
            }
        }
        ...
        createZoneHeader("🛡️ SAFE ZONE", "Low Risk • High Win Rate", "#4CAF50");
        ...
        if (safeMatches.size() >= 3) {
            ...
            layout.addView(createNoTicketMessage("Not enough safe matches today"));
        }
        ...
        if (mediumMatches.size() >= 4) {
            ...
            layout.addView(createNoTicketMessage("Not enough medium matches today"));
        }
        
        createZoneHeader("⚖️ BALANCED ZONE", "Best Risk/Reward Ratio", "#FF9800");
        ...
        if (riskyMatches.size() >= 5) {
            ...
            layout.addView(createNoTicketMessage("Not enough matches for risky ticket"));
        }
        ...
        if (smartMatches.size() >= 2) {
            ...
            layout.addView(createNoTicketMessage("Not enough matches for smart value ticket"));
        }
        
        if (valueMatches.size() >= 3) {
            layout.addView(createSmartTicketCard("💰 VALUE HUNTER (3-4 Matches - 15-25 Odds)", valueMatches.subList(0, Math.min(4, valueMatches.size())), "value"));
            saveSmartTicket("value", valueMatches.subList(0, Math.min(4, valueMatches.size())), today);
        } else {
            layout.addView(createNoTicketMessage("Not enough value matches today"));
        }
        
        createZoneHeader("🚀 HIGH REWARD ZONE", "Higher Risk • Massive Payout", "#F44336");
        
        if (extreme1Matches.size() >= 5) {
            layout.addView(createSmartTicketCard("🔥 EXTREME TIER 1 (5 Matches - 25-40 Odds)", extreme1Matches.subList(0, 5), "extreme1"));
            saveSmartTicket("extreme1", extreme1Matches.subList(0, 5), today);
        } else {
            layout.addView(createNoTicketMessage("Not enough matches for Extreme Tier 1"));
        }
        
        if (extreme2Matches.size() >= 7) {
            layout.addView(createSmartTicketCard("💥 EXTREME TIER 2 (7 Matches - 100+ Odds)", extreme2Matches.subList(0, 7), "extreme2"));
            saveSmartTicket("extreme2", extreme2Matches.subList(0, 7), today);
        } else {
            layout.addView(createNoTicketMessage("Not enough matches for Extreme Tier 2"));
        }
        
        prefs.edit().putString("last_ticket_date", today).apply();
    }
    
    private TextView createNoTicketMessage(String message) {
        TextView tv = new TextView(this);
    ...
    private void createZoneHeader(String title, String subtitle, String color) {
        LinearLayout headerLayout = new LinearLayout(this);
        headerLayout.setOrientation(LinearLayout.VERTICAL);
        headerLayout.setBackgroundColor(Color.parseColor(color));
        headerLayout.setPadding(25, 20, 25, 20);
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
            LinearLayout.LayoutParams.MATCH_PARENT,
            LinearLayout.LayoutParams.WRAP_CONTENT
        );
        ...
        subtitleView.setPadding(0, 5, 0, 0);
    ...
    private SmartPick getBestSmartPick(MatchPrediction m) {
        ...
        double htWinOdds = Math.min(m.homeOdds * 1.7, 3.5);
        ...
        double dcOdds = Math.max(m.winOdds * 0.7, 1.3);
    ...
    private SmartPick getBestValuePick(MatchPrediction m) {
        ArrayList<SmartPick> options = new ArrayList<>();
        
        double htWinOdds = Math.min(m.homeOdds * 1.75, 2.8);
        double htWinProb = m.prob * 0.64;
        if (htWinOdds >= 2.0 && htWinOdds <= 2.8 && htWinProb >= 60) {
            options.add(new SmartPick(m.homeTeam, m.awayTeam, 
                m.winner + " Win 1st Half", htWinOdds, (int)htWinProb));
        }
        
        double dcOdds = Math.min(Math.max(m.winOdds * 0.8, 1.8), 2.5);
        double dcProb = Math.min(m.prob + 10, 75);
        if (dcOdds >= 2.0 && dcOdds <= 2.8) {
            options.add(new SmartPick(m.homeTeam, m.awayTeam, 
                m.winner + " or Draw (DC)", dcOdds, (int)dcProb));
        }
        
        if (m.winOdds >= 2.0 && m.winOdds <= 2.8 && m.prob >= 60) {
            options.add(new SmartPick(m.homeTeam, m.awayTeam, 
                m.winner + " Win", m.winOdds, m.prob));
        }
        
        if (options.isEmpty()) {
            return new SmartPick(m.homeTeam, m.awayTeam, m.winner + " Win", 
                Math.min(m.winOdds * 1.3, 2.5), Math.max(m.prob - 5, 60));
        }
        
        SmartPick best = options.get(0);
        for (SmartPick pick : options) {
            if (pick.odds >= 2.0 && pick.odds <= 2.8 && pick.prob >= 62) {
                if (pick.odds > best.odds) {
                    best = pick;
                }
            }
        }
    ...
    private SmartPick getBestHighOddsPick(MatchPrediction m) {
        ArrayList<SmartPick> options = new ArrayList<>();
        
        double htWinOdds = Math.min(m.homeOdds * 1.8, 4.0);
        double htWinProb = m.prob * 0.63;
        if (htWinProb >= 55) {
            options.add(new SmartPick(m.homeTeam, m.awayTeam, 
                m.winner + " Win 1st Half", htWinOdds, (int)htWinProb));
        }
        
        double dcOdds = Math.max(m.winOdds * 0.75, 1.5);
        double dcProb = Math.min(m.prob + 12, 80);
        if (dcOdds >= 1.8) {
            options.add(new SmartPick(m.homeTeam, m.awayTeam, 
                m.winner + " or Draw (DC)", dcOdds, (int)dcProb));
        }
        ...
        if (options.isEmpty()) {
            return new SmartPick(m.homeTeam, m.awayTeam, m.winner + " Win", m.winOdds, m.prob);
        }
        
        SmartPick best = options.get(0);
        for (SmartPick pick : options) {
            if (pick.odds >= 2.0 && pick.prob >= 58) {
                if (pick.odds > best.odds) {
                    best = pick;
                }
            }
    ...
    private LinearLayout createSmartTicketCard(String title, java.util.List<SmartPick> picks, String type) {
        ...
        if (type.equals("value")) {
            card.addView(createInfoText("⭐ Best risk/reward ratio!", Color.parseColor("#FFD700")));
        }
        
        if (type.equals("extreme1") || type.equals("extreme2")) {
            card.addView(createInfoText("⚠️ High risk, high reward!", Color.parseColor("#F44336")));
        }
