...
public class MainActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        loading.setText("⚽ Loading matches...\n🤖 Checking past tickets...");
        ...
        executor.execute(() -> {
            try {
                ...
                if (!yesterday.isEmpty() && !yesterday.equals(today)) {
                    // Check if we have saved tickets from yesterday
                    String safeTicket = prefs.getString("saved_safe_" + yesterday, "");
                    String mediumTicket = prefs.getString("saved_medium_" + yesterday, "");
                    String riskyTicket = prefs.getString("saved_risky_" + yesterday, "");
                    
                    if (!safeTicket.isEmpty()) checkTicketResults("safe", safeTicket);
                    if (!mediumTicket.isEmpty()) checkTicketResults("medium", mediumTicket);
                    if (!riskyTicket.isEmpty()) checkTicketResults("risky", riskyTicket);
                    
                    // Clear old tickets
                    prefs.edit()
                        .remove("saved_safe_" + yesterday)
                        .remove("saved_medium_" + yesterday)
                        .remove("saved_risky_" + yesterday)
                        .apply();
                }
            } catch (Exception e) {
            }
    ...
    private void checkTicketResults(String type, String ticketData) {
        try {
            ...
            for (int i = 0; i < matches.length(); i++) {
                ...
                String homeTeam = match.getString("home");
                String awayTeam = match.getString("away");
                String predictedWinner = match.getString("prediction");
                
                // Fetch actual result (simplified - checks if match data available)
                String actualWinner = getActualResult(homeTeam, awayTeam);
    ...
    private String getActualResult(String home, String away) {
        // Simplified: In real implementation, fetch actual scores from API
        // For now, returns prediction (you'd implement full API call here)
    ...
    private static class MatchPrediction {
        ...
        MatchPrediction(String homeTeam, String awayTeam, String date, String dateLabel,
                       String winner, int prob, double odds, String predictedScore, String category, String time) {
